...
JLib::TaskDAG::BlockSlot& JLib::TaskDAG::EdgeSlotForThisThread() {
    Thread* self = TaskScheduler::IsInitialized()
...
JLib::DagEdge* JLib::TaskDAG::AllocEdge() {
    ...
    for (;;) {
        ...
        // mi_malloc, not a worker's theap: graph building has no task, so there is no identity to
        // pass here, and any thread may free the block later.
        // JLIB_DAG_CTL_EDGE_NEW: plain new/delete, for A/B only.
#if defined(JLIB_DAG_CTL_EDGE_NEW)
        EdgeBlock* fresh = new (std::nothrow) EdgeBlock();
        if (!fresh) return nullptr;
#else
        void* mem = mi_malloc(sizeof(EdgeBlock));
        if (!mem) return nullptr;
