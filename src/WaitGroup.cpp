...
{
    // PARK BEFORE PUBLISHING. The push below makes this task reachable, and a waker that finds it
    // resolves record->handle -- which does not exist until StoreTask has run. Parking afterwards
    // loses any wake that lands in between, and nothing can address the task after that.
