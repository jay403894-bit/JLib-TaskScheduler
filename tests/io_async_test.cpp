...
static JLib::Coro ReadHome(void* h, int times) {
    ...
    for (int i = 0; i < times; ++i) {
        JLib::Thread* at = JLib::TaskScheduler::SelfWorker(JLib::TaskScheduler::GetWorkers());   // null: main helping (unpinned by definition)
        ...
        if (at && JLib::TaskScheduler::SelfWorker(JLib::TaskScheduler::GetWorkers()) != at) g_ioMoved.fetch_add(1, std::memory_order_relaxed);
    }
}
...
int main(int argc, char** argv) {
    ...
    std::printf("a pending co_await is ended by cancellation, through its COMPLETION\n");
    {
        ...
        Reset();
        JLib::WaitGroup wg;
        JLib::Spawn(ReadOnce(server, 64, 0, op.Token()), &wg, op.Token().Raw());
    ...
    std::printf("hundreds of concurrent reads, all cancelled together\n");
    {
        ...
        for (int i = 0; i < kN; ++i)
            JLib::Spawn(ReadOnce(server, 8, 0, scope.Token()), &wg, scope.Token().Raw());
