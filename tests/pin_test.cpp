...
// Which worker am I on NOW. Called immediately after every suspension, which is exactly where
// thread-local state is untrustworthy: a TLS address computed before the switch names the thread
// this fiber WAS on, so a migration reads as "didn't move" -- the test under-reports the very
// thing it measures. Derive it: stack -> fiber -> task -> home, re-stamped at every dispatch.
static int Q() {
	// SelfWorker, not FiberFromStack directly. FiberFromStack says which STACK you are on, not
	// which worker -- getting from one to the other goes through record->home, which is merely a
	// Thread pointer with nothing checking it is still live. SelfWorker takes that hop AND
	// validates the result against the live workers array, which is the whole reason it exists.
	// In a test whose entire job is measuring worker identity, the unvalidated version is the
	// worst possible shortcut.
	if (Thread* self = TaskScheduler::SelfWorker(TaskScheduler::GetWorkers()))
		return (int)self->qIndex;
	return -1;
}
...
static void CurrentBody(void*) {
	for (int i = 0; i < 20; ++i) {
		...
		at = Q();     if (Fiber* f = FiberFromStack()) f->Yield(Pin::Current); Stayed(at);
...
// Directed placement is an OPERATION, not a pin: Thread::SendTo parks this task and the landing
// hands its handle to the named worker. A pin is an argument to a suspend, so it could never
// express this -- the switch would have to happen inside StampPin.
static void ThreadBody(void*) {
	const int target = g_target.load();
	for (int i = 0; i < 20; ++i) {
		if (!Thread::SendTo((uint16_t)target)) { g_wrong++; break; }
		if (Q() != target) g_wrong++;
	}
	g_runs++;
}
...
static Coro CoroThread(int target) {
	for (int i = 0; i < 20; ++i) {
		co_await SendTo{ (uint16_t)target };   // the coroutine form of Thread::SendTo
		if (Q() != target) g_wrong++;
	}
	g_runs++;
}
...
static void RunMany(void (*fn)(void*), int n) {
	...
	for (int i = 0; i < n; ++i) { Task* t = s.CreateTask(fn, nullptr, TaskType::Fiber); t->waitGroup = &wg; s.Push(t); }
	s.WaitFor(wg);
}
...
int main(int argc, char** argv) {
	...
	auto& s = TaskScheduler::Instance();
	std::printf("pin_test workers=%zu default=%s\n", s.GetWorkerCount(),
	...
	// Pin::None means STEALABLE -- land on a deque rather than a hi-pri inbox -- not "migrates".
	// ResumeFiber keeps an unpinned task on the waker's own deque when the waker is its home, and
	// places it otherwise; both are deques, so both honour the contract. Whether it actually ends
	// up on another worker depends on who happens to wake it and whether a thief takes it, and
	// neither is promised.
	//
	// There used to be a `g_moved > 0` check here. It passed for years because the old suspend
	// protocol requeued a waiter over and over, so a task got dozens of chances to be taken
	// elsewhere and migration was effectively guaranteed -- the spin was manufacturing the evidence
	// for an assertion the design never made. With a real park a task is woken once, and
	// moved=0 across 32 tasks is a legitimate outcome (observed 0 to 72 over 20 runs).
	// Retired rather than weakened: a probabilistic check that fails 1 run in 10 is worse than no
	// check, and the number is still worth seeing.
	std::printf("[fibers: Pin::None may move -- observation, not a check]\n");
	Reset(); RunMany(&NoneBody, 32);
	std::printf("  moved=%d of %d resumes\n", g_moved.load(), 32 * 20);
	Check(g_runs == 32, "every unpinned task ran to completion");
