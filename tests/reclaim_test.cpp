...
static void Holder(void*) {
	for (int k = 0; k < 20; ++k) {
		...
		if (!r) { g_empty.fetch_add(1); if (Fiber* f = FiberFromStack()) f->Yield(); continue; }
		g_acquired.fetch_add(1);
		Thread* before = TaskScheduler::SelfWorker(TaskScheduler::GetWorkers());
		for (int y = 0; y < 3; ++y) {
			if (Fiber* f = FiberFromStack()) f->Yield();
			...
			Thread* cur = TaskScheduler::SelfWorker(TaskScheduler::GetWorkers());
			const size_t other = (cur && cur->IsPoolWorker()) ? ((size_t)cur->qIndex + 1) % n : 0;
			s.PushBatch(&t, 1, other, 0);
			s.WaitFor(wg);
			if (r->magic != 0xA11CE) g_sawDead.fetch_add(1);
		}
		if (TaskScheduler::SelfWorker(TaskScheduler::GetWorkers()) != before) g_moved.fetch_add(1);
...
int main(int argc, char** argv) {
	...
	if (child) {
		...
		g_hzSrc.store(&dummy);
		WaitGroup wg; wg.n.store(1);
		Task* t = s.CreateTask(&HazardAcrossYield, nullptr, TaskType::Fiber);
	...
	std::printf("[retire storm, then idle]\n");
	{
		...
		wg.n.store(N * 2);
		for (int i = 0; i < N; ++i) {
			Task* f = s.CreateTask(&RetireSome, (void*)(intptr_t)(i * 7 + 1), TaskType::Fiber);   // fiber
	...
	std::printf("[Ref<T>: acquire under a guard, hold across yields and threads]\n");
	{
		...
		for (int i = 0; i < 2; ++i) { Task* w = s.CreateTask(&Writer, nullptr, TaskType::Fiber); w->waitGroup = &wwg; s.Push(w); }
		WaitGroup hwg;
		const int H = 64;
		hwg.n.store(H);
		for (int i = 0; i < H; ++i) { Task* h = s.CreateTask(&Holder, nullptr, TaskType::Fiber); h->waitGroup = &hwg; s.Push(h); }
		s.WaitFor(hwg);
