...
static void Roam(void*) {
	...
	for (int i = 0; i < kStops; ++i) {
		...
		if (home != Thread::GetCurrent()) g_badHome++;
		home->Local<Bucket>().n++;
		if (home->Local<Other>().tag != 7) g_badHome++;
		Thread::Yield(Pin::None);            // may resume on another worker
	}
}
...
int main() {
	...
	for (int i = 0; i < kTasks; ++i) {
		Task* t = s.CreateTask(&Roam, nullptr, Lane::Normal, TaskType::Fiber);
		t->waitGroup = &wg;
		s.Push(t);
	}
	s.WaitFor(wg);
	...
	long long sum = 0;
	int buckets = 0, wrongHeap = 0, misaligned = 0, shared = 0, neverBuilt = 0;
	...
	std::printf("  %d workers built a bucket; sum %lld of %d\n", buckets, sum, kTasks * kStops);
	...
	Check(sum == (long long)kTasks * kStops, "no increment lost: every worker wrote only its own bucket");
