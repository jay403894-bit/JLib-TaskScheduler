...
static void* CheckHere(Task* self, void* prev, unsigned char tag) {
	...
	Thread* now  = TaskScheduler::SelfWorker(TaskScheduler::GetWorkers());         // the reference it is compared against
	...
	if (prev) {
		...
		mi_free(prev);                               // cross-thread free
	}
	g_checks++;
	return p;
}
...
static void Hopper(void* arg) {
	...
	for (int i = 1; i <= kHops; ++i) {
		...
		Thread::SendTo(target);   // a yield with a named placement
		g_hops++;
...
static void Roamer(void*) {
	...
	for (int i = 1; i <= kRoams; ++i) {
		...
		if (Fiber* f = FiberFromStack()) f->Yield(Pin::None);                                // stolen wherever
		if (kept != self->record->home) g_staleSeen++;
		block = CheckHere(self, block, (unsigned char)i);
	}
	mi_free(block);
...
static void NativeOnce(void*) {
	...
	mi_free(CheckHere(self, nullptr, 0));
}
...
static Coro CoroRoam() {
	...
	for (int i = 1; i <= kCoroHops; ++i) {
		co_await SendTo{ (uint16_t)(i % g_workers) };   // forced migration
		block = CheckHere(self, block, (unsigned char)i);
	}
	mi_free(block);
}
...
static void RunAll(TaskScheduler& s, void (*fn)(void*), int n, TaskType type = TaskType::Fiber) {
	...
	for (int i = 0; i < n; ++i) {
		Task* t = s.CreateTask(fn, (void*)(intptr_t)i, type);
		t->waitGroup = &wg;
		s.Push(t);
	}
	s.WaitFor(wg);
