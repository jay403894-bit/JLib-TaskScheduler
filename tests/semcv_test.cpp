...
static int Q() { Thread* t = TaskScheduler::SelfWorker(TaskScheduler::GetWorkers()); return t ? (int)t->qIndex : -1; }
...
static void Spawn(void (*fn)(void*), int n, WaitGroup& wg, uint32_t token = CancelToken::kNone) {
	...
	wg.n.fetch_add(n);
	for (int i = 0; i < n; ++i) {
		Task* t = s.CreateTask(fn, nullptr, TaskType::Fiber);
		t->waitGroup = &wg;
		t->cancelToken = token;
		s.Push(t);
	}
...
int main(int argc, char** argv) {
	...
	std::printf("[a cancelled coroutine waiter resumes where it suspended]\n");
	{
		...
		for (int i = 0; i < 64; ++i) {
			Coro c = CoroAcquire();
			Spawn(std::move(c), &wg, scope.Token().Raw());
		}
