...
static void MainOnly(void*) {
	...
	for (int i = 0; i < 10; ++i) {
		...
		s.WaitFor(wg);                           // resumes where it parked -- a resume does not route
		Thread::SendToMain();                    // the hop is an operation, not a pin
		if (std::this_thread::get_id() != g_mainId) g_offMain++;
		if (Fiber* f = FiberFromStack()) f->Yield(Pin::Main);
	}
	g_done++;
}
...
int main() {
	...
	for (int i = 0; i < kTasks; ++i) {
		Task* t = s.CreateTask(&MainOnly, nullptr, TaskType::Fiber);
		t->waitGroup = &all;
