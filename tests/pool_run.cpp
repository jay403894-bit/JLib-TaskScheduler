...
static void FiberWaiterBody(void* p) {
	...
	JLib::Task* t = c.sched->CreateTask(&Body, nullptr, JLib::Lane::Normal, JLib::TaskType::Fiber);
	t->waitGroup = c.inner;
	c.sched->Push(t);
...
int main(int argc, char** argv) {
	const bool useK = HasFlag(argc, argv, 'k');
	...
	cfg.workers    = 4;
	cfg.hotWorkers = useK ? 2 : 0;
	Done(useK ? "K = 2" : "K = 0");

	Stage("Init(4)");
	JLib::TaskScheduler::Init(cfg);
	auto& sched = JLib::TaskScheduler::Instance();
	std::fprintf(stderr, "        K = %zu (reserved band is the tail of the queue range)\n",
		JLib::TaskScheduler::GetHotWorkers());
	std::fflush(stderr);
	if (useK && JLib::TaskScheduler::GetHotWorkers() == 0) {
		std::fprintf(stderr,
			"        FAIL: asked for K=2 and got 0. Config::hotWorkers did not survive Init --\n"
			"        this is the mechanism the 'K is lost' story was used to disable.\n");
		std::fflush(stderr);
		return 1;
	}
	...
	Stage("one Native task + WaitFor");
	{
		...
		JLib::Task* t = sched.CreateTask(&Body, nullptr, JLib::Lane::Normal, JLib::TaskType::Fiber);
		t->waitGroup = &wg;
		sched.Push(t);
		sched.WaitFor(wg);
	}
	...
	Stage("DAG: diamond A -> {B,C} -> D");
	{
		...
		JLib::Task* ta = sched.CreateTask(&Body, nullptr, JLib::Lane::Normal, JLib::TaskType::Fiber);
		JLib::Task* tb = sched.CreateTask(&Body, nullptr, JLib::Lane::Normal, JLib::TaskType::Fiber);
		JLib::Task* tc = sched.CreateTask(&Body, nullptr, JLib::Lane::Normal, JLib::TaskType::Fiber);
		JLib::Task* td = sched.CreateTask(&Body, nullptr, JLib::Lane::Normal, JLib::TaskType::Fiber);
	...
	Stage("DAG: 1 -> 256 -> 1 fan-out/fan-in");
	{
		...
		JLib::Task* tr = sched.CreateTask(&Body, nullptr, JLib::Lane::Normal, JLib::TaskType::Fiber);
		JLib::Task* tj = sched.CreateTask(&Body, nullptr, JLib::Lane::Normal, JLib::TaskType::Fiber);
	...
	Stage("DAG: AddDependency after Submit, predecessor already finished");
	{
		...
		JLib::Task* ta = sched.CreateTask(&Body, nullptr, JLib::Lane::Normal, JLib::TaskType::Fiber);
		JLib::Task* tl = sched.CreateTask(&Body, nullptr, JLib::Lane::Normal, JLib::TaskType::Fiber);
		ta->waitGroup = &aDone;
	...
	Stage("DAG: AddDependency racing the completion walk");
	{
		...
		for (int it = 0; it < kIters; ++it) {
			...
			JLib::Task* ta = sched.CreateTask(&Body, nullptr, JLib::Lane::Normal, JLib::TaskType::Fiber);
			JLib::Task* tl = sched.CreateTask(&RaceLateBody, &ctx, JLib::Lane::Normal, JLib::TaskType::Fiber);
			ta->waitGroup = &aDone;
	...
	Stage("Lane::LowLatency reachability (intake producer on, consumer set may be empty)");
	{
		const size_t k = JLib::TaskScheduler::GetHotWorkers();
		std::atomic<int> lowRan{ 0 };
		struct LowCtx { std::atomic<int>* n; };
		LowCtx ctx{ &lowRan };
		auto body = +[](void* p) { static_cast<LowCtx*>(p)->n->fetch_add(1, std::memory_order_release); };

		JLib::Task* t = sched.CreateTask(body, &ctx, JLib::Lane::LowLatency, JLib::TaskType::Fiber);
		sched.Push(t);

		for (int spin = 0; spin < 400000 && lowRan.load(std::memory_order_acquire) == 0; ++spin)
			std::this_thread::yield();
		...
		if (got == 0) {
			std::fprintf(stderr,
				"        STRANDED: a Lane::LowLatency task never ran. The intake producer is on\n"
				"        and its only consumer requires isReservedWorker, which is empty at K=0.\n");
			std::fflush(stderr);
			return 1;
		}
	}
	Done("a LowLatency task reached a worker");
