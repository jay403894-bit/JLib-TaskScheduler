...
static void Waiter(void* p) {
    ...
    for (int i = 0; i < kWaitTimes; ++i) {
        ...
        if (JLib::TaskScheduler::SelfWorker(JLib::TaskScheduler::GetWorkers()) != at) g_waitMoved.fetch_add(1, std::memory_order_relaxed);
    }
}
...
int main(int argc, char** argv) {
    ...
    std::printf("IoReactor from fibers -- workers=%zu %s\n\n", sched.GetWorkerCount(),
                pin ? "Pinned" : "Migrate");
    ...
    std::printf("continuation tasks (the resume is a fresh fiber)\n");
    {
        ...
        for (int i = 0; i < kOps; ++i) {
            ...
            JLib::Task* t = sched.CreateTask(&OnRead, &ops[i], JLib::TaskType::Fiber);
            t->waitGroup = &wg;
    ...
    std::printf("fibers that wait for their reads (%d x %d)\n", kWaiters, kWaitTimes);
    {
        ...
        for (int i = 0; i < kWaiters; ++i) {
            ctx[i] = WaiterCtx{ h, i };
            JLib::Task* t = sched.CreateTask(&Waiter, &ctx[i], JLib::TaskType::Fiber);
            t->waitGroup = &wg;
            sched.Push(t);
        }
        sched.WaitFor(wg);
    ...
    std::printf("a read that never completes is cancelled\n");
    {
        ...
        JLib::Task* t = sched.CreateTask(&OnCancelled, &op, JLib::TaskType::Fiber);
        t->waitGroup = &wg;
