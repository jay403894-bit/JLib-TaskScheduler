...
static void PostToMain(void* p) {
	...
	for (int i = 0; i < c.n; ++i) {
		Task* t = s.CreateTask(&MustBeMain, nullptr, Lane::Normal, c.type);
		t->waitGroup = c.wg;
		s.PushMain(t);
	}
}
...
static void NestedWaiter(void*) {
	...
	for (int i = 0; i < 64; ++i) {
		Task* t = s.CreateTask(&Counted, nullptr, Lane::Normal, TaskType::Fiber);
		t->waitGroup = &inner;
		s.Push(t);
	}
	s.WaitFor(inner);
...
static void PostBlockingToMain(void* p) {
	...
	auto& s = TaskScheduler::Instance();
	for (int i = 0; i < c.n; ++i) {
		Task* t = s.CreateTask(&MainBlocking, nullptr, Lane::Normal, c.type);
		t->waitGroup = c.wg;
		s.PushMain(t);
	}
}
...
int main(int argc, char** argv) {
	const bool useK = argc > 1 && std::strchr(argv[1], 'k');
	...
	std::printf("FTL workers=%zu K=%zu fibers=%s\n", n, TaskScheduler::GetHotWorkers(), pin ? "Pin" : "Migrate");

	g_stageSeq = 1; Stage("main is slot 0");
	{
		Thread* me = Thread::GetCurrent();
		Check(me != nullptr && me->qIndex == 0 && me->isMain, "Thread::GetCurrent() on main is slot 0");
	...
	g_stageSeq = 2; Stage("PushTo(q) runs on q; epoch slot q");
	{
		...
		for (size_t q = 0; q < n; ++q) {
			Task* t = s.CreateTask(&Targeted, (void*)(intptr_t)q, Lane::Normal, TaskType::Fiber);
			t->waitGroup = &wg;
	...
	g_stageSeq = 4; Stage(ty == TaskType::Fiber ? "PushMain from workers (Fiber)" : "PushMain from workers (Native)");
	{
		...
		for (int i = 0; i < posters; ++i) {
			Task* t = s.CreateTask(&PostToMain, &ctx[i], Lane::Normal, TaskType::Fiber);
			t->waitGroup = &wg;
	                                             ...
	                                             : "PushMain tasks that WaitFor stay on main (Native: main helps)");
	{
		...
		std::vector<PostCtx> ctx(posters, PostCtx{ &wg, per, ty });
		for (int i = 0; i < posters; ++i) {
			Task* t = s.CreateTask(&PostBlockingToMain, &ctx[i], Lane::Normal, TaskType::Fiber);
			t->waitGroup = &wg;
			s.PushTo(1 + (size_t)i % (n - 1), t);
		}
		s.WaitFor(wg);
	...
	g_stageSeq = 5; Stage("suspending fibers (some on main)");
	{
		...
		WaitGroup wg; wg.n.store(N);
		for (int i = 0; i < N; ++i) {
			Task* t = s.CreateTask(&SuspendingFiber, nullptr, Lane::Normal, TaskType::Fiber);
			t->waitGroup = &wg;
	...
	g_stageSeq = 6; Stage("nested WaitFor: Native waiters on main, Fiber waiters on workers");
	{
		...
		for (int i = 0; i < 8; ++i) {
			...
			Task* t = s.CreateTask(&NestedWaiter, nullptr, Lane::Normal, TaskType::Fiber);
			t->waitGroup = &wg;
	...
	g_stageSeq = 10; Stage("DAG with a main node");
	{
		...
		Task* tm = s.CreateTask(&MustBeMain, nullptr, Lane::Normal, TaskType::Fiber); tm->waitGroup = &wg;
	...
	Check(Thread::GetCurrent() == nullptr, "main's slot released");
