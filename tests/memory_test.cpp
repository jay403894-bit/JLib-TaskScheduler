...
static void AllocOne(void* p) {
	...
	unsigned char* b = static_cast<unsigned char*>(Alloc(n));
	if (!b) { g_bad++; return; }
...
static void RunPool(const char* mode, bool keepOne) {
	...
	void* mainBlock = Alloc(256);            // main allocates too
	NoteHeap();
	...
	const bool mainHasHeap = Thread::GetCurrent() && Thread::GetCurrent()->heap;
	...
	void* survivor = keepOne ? Alloc(4096) : nullptr;
	Free(mainBlock);
