...
int main(int argc, char** argv) {
	...
	std::printf("main out of pool, workers=%zu, K=%zu, inPool=%d\n",
		n, TaskScheduler::GetHotWorkers(), (int)(TaskScheduler::GetMainMode() == MainMode::InPool));

	std::printf("[main]\n");
	Check(thread_id == 0, "main has epoch slot 0");
	{
		Thread* m = Thread::GetCurrent();
	...
	std::printf("[spread: 20000 tasks via Push]\n");
	{
		...
		s.WaitFor(wg);
		int total = 0;
		const size_t computeN = n - TaskScheduler::GetHotWorkers();
		bool allCompute = true;
		for (size_t q = 0; q < n; ++q) {
			const int c = g_perWorker[q].load();
			total += c;
			std::printf("  worker %zu ran %d%s\n", q, c, q >= computeN ? "  (reserved)" : "");
			if (q < computeN && c == 0) allCompute = false;
		}
