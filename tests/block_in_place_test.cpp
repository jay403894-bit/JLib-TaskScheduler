...
static void BlockerBody(void*) {
	...
	g_blockerQ.store(TaskScheduler::SelfWorker(TaskScheduler::GetWorkers())->qIndex);
	Task* ts[kPreQueued];
	for (int i = 0; i < kPreQueued; ++i) ts[i] = s.CreateTask(&Body, nullptr);
	s.PushBatch(ts, kPreQueued, (size_t)TaskScheduler::SelfWorker(TaskScheduler::GetWorkers())->qIndex, 0);
