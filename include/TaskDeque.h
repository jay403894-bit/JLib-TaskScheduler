...
namespace JLib {
    ...
    public:
        ...
        explicit TaskDeque(size_t capacity = 32768, size_t maxCapacity = kDefaultMaxCapacity)
            ...
            Ring* r = MakeRing(capacity);
            if (!r) throw std::bad_alloc();
            ring_.store(r, std::memory_order_relaxed);
            top_.store(0, std::memory_order_relaxed);
            bottom_.store(0, std::memory_order_relaxed);
        }

        ~TaskDeque() {
            for (Ring* r : retired_) FreeRing(r);
            FreeRing(ring_.load(std::memory_order_relaxed));
        }
        ...
        // MIMALLOC, NOT THE CRT HEAP. This used to be two allocating new-expressions, and nothing in
        // this library overrides operator new -- so the one structure the pool touches on every push
        // and pop was the one allocated outside the pool's own heap, and outside the arena that
        // Config::arenaReserveBytes exists to reserve. It matters at runtime and not just at start:
        // grow() runs on the OWNER's push path, so the new ring now comes from that worker's own
        // mimalloc heap rather than a process-wide one every worker shares.
        //
        // One block, header and slots together: they are read on the same operations, so there is
        // no reason for them to be two allocations that can land anywhere relative to each other.
        static Ring* MakeRing(size_t capacity) noexcept {
            constexpr size_t kSlotAlign = alignof(std::atomic<uintptr_t>);
            const size_t hdr   = (sizeof(Ring) + kSlotAlign - 1) & ~(kSlotAlign - 1);
            const size_t bytes = hdr + capacity * sizeof(std::atomic<uintptr_t>);

            void* mem = mi_malloc_aligned(bytes, platform::kCacheLine);
            if (!mem) return nullptr;

            auto* slots = reinterpret_cast<std::atomic<uintptr_t>*>(static_cast<char*>(mem) + hdr);
            for (size_t i = 0; i < capacity; ++i)
                ::new (static_cast<void*>(&slots[i])) std::atomic<uintptr_t>(0);
            return ::new (mem) Ring{ capacity - 1, capacity, slots };
        }

        // std::atomic<uintptr_t> and Ring are trivially destructible, so the block is the object.
        static void FreeRing(Ring* r) noexcept { if (r) mi_free(r); }
        ...
        void grow(Ring* old, size_t t, size_t b) {
            ...
            Ring* r = MakeRing(newCap);   // noexcept now: null is the failure, not an exception
            if (!r) FatalGrow(old->capacity, "out of memory", ownerIndex_, ownerLane_, t, b);
