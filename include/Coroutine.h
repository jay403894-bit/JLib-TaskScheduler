...
#define JLIB_CORO_FRAME_ALLOC                                                                   \
    ...
    inline bool Spawn(Coro&& c, WaitGroup* wg = nullptr,
        ...
        Task* t = sched.CreateTask(&detail::ResumeCoroutine,
                                   h.address(), lane, TaskType::Coroutine);
    ...
    public:
        ...
        bool await_suspend(std::coroutine_handle<P> h) {
            ...
            if (IsTaskCancelled(task_)) return false;   // an Event never unlinks a single waiter
            ev_.AddWaiter(task_);                       // may resume elsewhere before this returns
            return true;
        }
